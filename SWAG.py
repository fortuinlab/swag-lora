import torch
from utils.peft_utils import get_modules_with_grad
import numpy as np
import itertools
...
### Code adapted from swa_gaussian repo (https://github.com/wjmaddox/swa_gaussian)
### Main difference is to only collect SWAG parameters from LoRA layers
...
class SWAG(torch.nn.Module):
    def __init__(
        self, base, no_cov_mat=True, max_num_models=0, var_clamp=1e-30, modules_to_swag='grad_only', *args, **kwargs
    ):
        super(SWAG, self).__init__()

        self.register_buffer("n_models", torch.zeros([1], dtype=torch.long))
        self.params = list()
        self.buffer_names = list()
        self.no_cov_mat = no_cov_mat
        self.max_num_models = max_num_models
        self.var_clamp = var_clamp

        # select which modules to swag: 'all' (all leaf modules with parameters), 'grad_only' (only modules requiring gradients), 'lora_only' (only lora layers)
        if modules_to_swag not in ['grad_only', 'all', 'lora_only']:
          raise Exception('Swag modules must be "grad_only", "lora_only" or "all".')
        self.modules_to_swag = modules_to_swag

        self.base = base # changed it to just accept an initialized/trained model
        if modules_to_swag == 'grad_only':
          self.module_names = [name for (name, module) in get_modules_with_grad(self.base)]
        elif modules_to_swag == 'lora_only':
          self.module_names = [name for (name, module) in get_modules_with_grad(self.base) if 'lora' in name]

        self.init_swag_model()

    def is_swag_module(self, name, module):
      if not module._parameters \
      or list(module.children())  \
      or (self.modules_to_swag in {'grad_only', 'lora_only'} and name not in self.module_names):
        return False
      return True

    def init_swag_model(self):
      for name, module in self.base.named_modules():
        if not self.is_swag_module(name, module):
          continue

        for param_name in list(module._parameters.keys()):
          if module._parameters[param_name] is None:
            continue

          data = module._parameters[param_name].data

          module.register_buffer("%s_mean" % param_name, data.new(data.size()).zero_())
          module.register_buffer("%s_sq_mean" % param_name, data.new(data.size()).zero_())

          if self.no_cov_mat is False:
            module.register_buffer(
                "%s_cov_mat_sqrt" % param_name, data.new_empty((0, data.numel())).zero_()
            )

          buffer_name = name.replace('.', '-') + '-' + param_name
    ...
    def sample_blockwise(self, scale, cov, fullrank):
        for module, name in self.params:
            ...
            getattr(module, name).data = w.to(device).data
    ...
    def sample_fullrank(self, scale, cov, fullrank):
        ...
        for (module, name), sample in zip(self.params, samples_list):
            device = next(module.parameters()).device
            getattr(module, name).data = sample.to(device).data

    def collect_model(self, base_model):
        # get the swag parameters from base model
        base_model_params = []
        for name, module in base_model.named_modules():
          ...
          for param_name in module._parameters:
            if module._parameters[param_name] is None:
              continue
            base_model_params.append(module._parameters[param_name])

        # update SWAG model
        for i, ((module, name), base_param) in enumerate(zip(self.params, base_model_params)):
            mean = module.__getattr__("%s_mean" % name)
            sq_mean = module.__getattr__("%s_sq_mean" % name)
